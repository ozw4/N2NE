...
from util import Instantaneous_scaling, load_seismic_data, reshape_traces,  calculate_psnr
...
gain_factor = np.zeros_like(traces)
scaled_traces = np.zeros_like(traces)

# Instantaneous scaling
for i in range(sweep_n):
    for j in tqdm(range(traces.shape[1])):
         scaled_traces[i, j], gain_factor[i, j] = Instantaneous_scaling(traces[i, j], window_leng)
          
scaled_traces = np.transpose(scaled_traces, (1, 0, 2, 3))
flt_stack = np.mean(scaled_traces, axis=1) / np.mean(gain_factor, axis=0)
