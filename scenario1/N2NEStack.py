...
from util import load_seismic_data, reshape_traces, torch_fix_seed, UNet, sweep_dataload, calculate_psnr
...
x_valid, t_valid = sweep_dataload(traces_valid, sweep_n, len(traces_valid))
...
if TRAIN:
    ...
    while True:
        itera += 1
        x_train, t_train = sweep_dataload(traces_train, sweep_n, batchsize)
...
# Denoising seismic traces and calculating metrics
flt_stack = np.zeros_like(original_traces)

for n in range(sweep_n):
    for i, x in enumerate(traces[:, n]):
